...
FoodLogger/                     ← git root & Xcode project root
  FoodLogger/                   ← app source (file-system sync root)
    ...
    Views/TodayTabView.swift      ← Tab 1 (Journal): owns @State selectedIndex; NavigationStack wrapping DayLogView + dark editorial banner (3px orange masthead strip, brandVoid+gradient bg, ghost day-number watermark, ALL CAPS amber context label, appTitleSerif date, caption entry count, vertical streak flame+count right); bottom orange rule; flame color: brandWarm <7, orange 7–13, red 14+
    ...
    Views/CalendarTabView.swift   ← Tab 2 (Calendar): full-screen calendar on brandVoid background; month header uses appDisplaySerif cream name + amber year + circular nav buttons; day-of-week row in amber rounded; day cells: heatmap (brandAccent opacity 0→0.22→0.50→0.80 for 0/1/2/3+ entries), selected=brandAccent fill, today=cream ring; brandAccent divider between grid and day panel; day entries panel uses appHeadlineSerif title; CalendarTabDayCell takes entryCount: Int
    ...
    Views/FoodsTabView.swift      ← Tab 3 (Foods): NavigationStack + @Query allEntries + .searchable bar (always-visible drawer); lists all food tokens (topItems period:.allTime limit:200) sorted by frequency, each row shows capitalized name + orange count Capsule; tapping a row pushes FoodItemTimelineView; empty state for zero entries
    Views/SettingsView.swift      ← Tab 5 (Settings): daily reminder toggle + time picker + JSON export; #if DEBUG "Clear Sample Data" + "Clear & Re-seed" section
    ...
    Views/FoodItemTimelineView.swift ← Push destination from FoodsTabView; no own NavigationStack (uses parent's); shows 4 charts: occurrence timeline (BarMark per day), day-of-week pattern (Mon–Sun bars), time-of-day distribution, mood distribution (horizontal BarMark per MoodTag, colored by MoodTag.color, with count+% annotation); has its own period picker (7D/30D/3M/1Y/All) and header stats row (total count, last eaten, count in period); uses same chartCard style as InsightsView
  ...
  docs/                         ← Marketing/docs site (landing page, privacy page, screenshots); no build target; canonical site
```
...
The app uses a **5-tab bottom tab bar** as the primary navigation structure:

- **Tab 1 — Journal** (`TodayTabView`): Owns `@State selectedIndex: Int` (lifted from `DayLogView`) so the banner reacts synchronously to page swipes. `NavigationStack` wrapping `DayLogView`. Adds a banner via `.safeAreaInset(edge: .top)` showing context label (greeting when on today, "Yesterday"/"X days ago" when on a past day), the **viewed day's date** in serif (not today's date), streak flame+count, and entry count for the viewed day. Bottom rule is vivid orange on today, dim amber on past days.
...
- **Tab 3 — Foods** (`FoodsTabView`): Owns `@Query allEntries` directly. `NavigationStack` with always-visible `.searchable` bar. Lists all food tokens (`topItems(period:.allTime, limit:200)`) ranked by frequency. Tapping a row pushes `FoodItemTimelineView` for per-item occurrence analytics.
- **Tab 4 — Insights** (`InsightsView`): Analytics dashboard. `AppShellView` owns `@Query allEntries` and passes it directly. No food search — that feature lives in Tab 3.
- **Tab 5 — Settings** (`SettingsView`): Settings. `AppShellView` computes `hasLoggedToday` from `StreakService` and passes it.
...
- **`DayLogView` (shell):** receives `@Binding var selectedIndex: Int` from `TodayTabView` (state is owned by the parent). Toolbar: search button + today pill/button. No streak badge in toolbar — streak lives in the banner. Nav title is `""` with `.inline` display mode (banner serves as the visual header).
...
- **InsightsService**: `@MainActor final class`. Accepts `[FoodEntry]`, returns typed analytics structs. Key types: `AnalyticsPeriod` (week/month/threeMonths/year/allTime), `FoodItemFrequency`, `DailyCount`, `CategoryCount`, `InputTypeCount`, `HourCount`, `WeekComparison`, `DayActivity`, `ItemPair`, `WeekdayCount`, `MoodCount` (all `Identifiable` except `WeekComparison`). General methods: `topItems`, `dailyCounts`, `categoryDistribution`, `inputTypeBreakdown`, `mealTiming`, `weekOverWeekTrend`, `monthlyHeatmap`, `coOccurrence`, `moodDistribution`. `moodDistribution(from:period:)` returns `[MoodCount]` (mood, count, percentage) sorted by frequency. Food item timeline methods: `entriesMatching(term:from:)` (case-insensitive substring match), `itemDailyCounts(for:from:period:)` (gap-filled daily occurrence counts), `itemMealTiming(for:from:period:)` (hour-of-day distribution), `itemWeekdayPattern(for:from:period:)` (Mon–Sun occurrence counts as `[WeekdayCount]`), `itemMoodDistribution(for:from:period:)` (mood breakdown as `[MoodCount]`, sorted by frequency). Strips stopwords: a, the, and, with, of, in, for, had, ate, some, my, an.
- **SampleDataService**: `@MainActor final class`. Two public methods: `seedIfNeeded(context:)` — no-op if any `FoodEntry` exists (used on first launch); `seed(context:)` — unconditional, always inserts a full batch (used by "Clear & Re-seed" in Settings). Seeds 120 days of data (~85% of days have 1–4 entries), all 6 categories, all 3 input types, 66 food items (12 beverage/breakfast/lunch/dinner, 10 snack, 8 dessert), all 5 `MoodTag` values (~20% nil, rest weighted satisfied→energised→neutral→sluggish→uncomfortable), ~10% of entries have `updatedAt` set. Voice entries use 7 varied transcript templates. Every `rawInput` prefixed with `[SAMPLE]`. Uses deterministic LCG (`SeededRNG`) for reproducible output. Image entries have `mediaURL = nil`.

## FoodsTabView architecture
Presented as **Tab 3**. Owns `@Query private var allEntries: [FoodEntry]` and an `InsightsService` instance. Background: `Color.brandVoid`.

- **Search:** `.searchable(placement: .navigationBarDrawer(displayMode: .always))` — search bar is always visible (no collapse). Filters `allFoods` list by case-insensitive substring match on `searchText`.
- **Food list:** `List` with `.listStyle(.plain)` + `.scrollContentBackground(.hidden)`. Each row: food name (`.appBody`, capitalized) + frequency count in an orange `Capsule` badge. `NavigationLink(destination: FoodItemTimelineView(term: food.item, entries: allEntries))` — uses push navigation, no sheet.
- **FoodItemTimelineView** has no own `NavigationStack` — it renders as a push destination inside `FoodsTabView`'s `NavigationStack`. Charts: occurrence timeline, day-of-week pattern, time-of-day distribution, mood distribution.
- **Empty state:** shown when `allEntries.isEmpty`.
- **No duplicate food search** in InsightsView — that card was removed.

## InsightsView architecture
Presented as **Tab 4** in the bottom tab bar. `AppShellView` owns `@Query allEntries` and passes it as `entries: [FoodEntry]`. Background: `Color.brandVoid` on the `NavigationStack` and `ScrollView`. **No food search** — that feature was moved to the dedicated Foods tab.
...
- **Cards rendered in order:** storyHeadlineCard → periodPicker → statsCard → recordsCard → topFoodsCard → dailyActivityCard → categoryCard → moodCard → mealTimingCard → weekTrendCard → heatmapCard
...
- `longestStreak`, `mostLoggedDayCount`, `totalUniqueFoods` computed all-time from the full `entries` array — not filtered by `selectedPeriod`; `totalUniqueFoods = service.topItems(from: entries, period: .allTime, limit: 200).count`
